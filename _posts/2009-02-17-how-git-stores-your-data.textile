...
The most basic data storage is the blob. Git stores *just* the contents of the file, and not the name. The contents are then referenced by a 40 character "SHA1 hash":http://en.wikipedia.org/wiki/SHA1 of the contents, which means it's pretty much guaranteed to be unique. Pretty much every object, be it a commit, tree, or blob has a SHA, so learn to love them. Luckily, they're easily referenced by the first 6 characters which are usually enough to identify the whole string.
