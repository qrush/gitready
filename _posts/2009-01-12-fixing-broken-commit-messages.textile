...
Someone just committed and pushed a commit that just isn't right. Either some information isn't filled out, the commit message is wrong, or something else is just messed up. Let's go over what can be done to fix the associated data after the fact.
...
That will generate files in the form of 0001-commit-message that contains the commit diff and metadata. One note with the 3 caret symbols: just add one for each commit you need to go back, and be consistent! Go ahead and edit these files so they have the proper information. Once that's done, we'll need to reset our repository back a few commits:
...
If you don't add the @-f@, git will complain. Just be aware that modifying commit messages may muck up other's repositories and should be used with caution. 
